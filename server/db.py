...
async def init_db() -> None:
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        for stmt in (
            ...
            "ALTER TABLE experiments ADD COLUMN training_tokens INTEGER",
            "ALTER TABLE experiments ADD COLUMN training_seconds REAL",
...
async def list_agent_bests(
    ...
    if exclude:
        ...
        cursor = await conn.execute(
            "SELECT agent_id, experiment_id as id, experiment_id, algorithm_code, "
            "       score, feasible, num_vehicles, total_distance, route_data, updated_at "
            "FROM agent_bests WHERE feasible = 1 ORDER BY score DESC"
        )
...
async def compute_leaderboard(
    ...
    cursor = await conn.execute(
        ...
        ORDER BY best_score IS NULL, best_score DESC, a.name ASC
        """
    )
    rows = await cursor.fetchall()
    return [
