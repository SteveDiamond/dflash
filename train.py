...
BLOCKS_PER_SEQ = 1            # training blocks sampled per sequence (each step
                              # does BATCH_SIZE*BLOCKS_PER_SEQ draft blocks from
                              # one target forward — handy once target-forward
                              # cost dominates)
SEED = 42                     # random seed
...
# Mask token
MASK_TOKEN_ID = 151669        # token ID used for masked positions (matches released oracle)
...
for step in range(NUM_STEPS):
    ...
    # Response tokens were pre-generated by target in prepare.py, so labels
    # from iids already match what eval measures. No need for an extra target
    # forward to regenerate labels during training.
